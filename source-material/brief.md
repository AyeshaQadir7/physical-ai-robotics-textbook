# Source Material Brief: Physical AI & Humanoid Robotics Textbook
...
- **The Inner Ear (Balance):** Generic USB IMU (BNO055).
  - _Note:_ Often built into the RealSense D435i or Jetson boards, but a separate module helps teach IMU calibration.
...
To successfully teach this course, the lab infrastructure is structured around four core components designed for simultaneous simulation and physical deployment:
...
If **RTX-enabled workstations** are not accessible, the course must be restructured to rely entirely on cloud-based instances (like AWS RoboMaker or NVIDIA's cloud delivery for Omniverse), although this introduces significant latency and cost complexity.

Building a "Physical AI" lab is a significant investment, requiring a choice between building a physical **On-Premise Lab** (High **CapEx**—Capital Expenditure) versus running a **Cloud-Native Lab** (High **OpEx**—Operational Expenditure).
...
Instead of buying PCs, you rent powerful instances.

- **Instance Type:** AWS **g5.2xlarge** (A10G GPU, 24GB VRAM) or **g6e.xlarge**.
- **Software:** NVIDIA Isaac Sim on **Omniverse Cloud** (requires specific AMI).
- **Cost Calculation (Per Student/Quarter):**
  - Instance cost: ~$1.50/hour (spot/on-demand mix).
  - Usage: 10 hours/week $\times$ 12 weeks = 120 hours.
...
This kit is optimized for student learning, focusing on **ROS 2**, **Basic Computer Vision**, and **Sim-to-Real control**, while keeping costs manageable.

| Component      | Model                                       | Price (Approx.)   | Notes                                                                   |
| :------------- | :------------------------------------------ | :---------------- | :---------------------------------------------------------------------- |
| **The Brain**  | NVIDIA Jetson Orin Nano Super Dev Kit (8GB) | **$249**          | New official MSRP. Capable of **40 TOPS** (Tera Operations Per Second). |
...
The difference between cloud-based simulation and physical robot control introduces a critical engineering problem known as the **Latency Trap**.

- **The Problem:** Simulating in the cloud works well, but controlling a **real robot** from a cloud instance is dangerous and impractical due to high network **latency**. Commands sent from the cloud take too long to reach the robot, leading to delayed reactions and instability.
- **The Solution:** Students adopt the industry standard for embodied AI:
  1.  **Train** the AI model in the high-performance **Cloud** or **Sim Rig**.
  2.  **Download** the resulting model (weights).
  3.  **Flash** the model onto the **local Jetson kit** (The Edge Brain) for low-latency, real-time control.
